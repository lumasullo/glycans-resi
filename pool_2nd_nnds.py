...
all_2nn_arrays_distances = []
all_2nn_arrays_distances_csr = []
...
for folder_name in sorted(string_list):
    ...
    if os.path.isdir(folder_path):
        ...
        if os.path.exists(csv_path_distances):
            ...
            if '2nn' in df_distances.columns:
                all_2nn_arrays_distances.append(df_distances['2nn'].values)
            else:
        ...
        if os.path.exists(csv_path_distances_csr):
            ...
            if '2nn' in df_distances_csr.columns:
                all_2nn_arrays_distances_csr.append(df_distances_csr['2nn'].values)
            else:
                print(f"'2nn' not found in {csv_path_distances_csr}")
        else:
            print(f"{csv_path_distances_csr} does not exist.")

# Concatenate all "1nn" arrays into single arrays for each file
combined_2nn_array_distances = np.concatenate(all_2nn_arrays_distances) if all_2nn_arrays_distances else np.array([])
combined_2nn_array_distances_csr = np.concatenate(all_2nn_arrays_distances_csr) if all_2nn_arrays_distances_csr else np.array([])

# Print the results for verification
print("Combined 2nn array from distances.csv:", combined_2nn_array_distances)
print("Combined 2nn array from distances_csr.csv:", combined_2nn_array_distances_csr)
...
# Define data arrays
data_distances = combined_2nn_array_distances
data_distances_csr = combined_2nn_array_distances_csr
...
N = len(all_2nn_arrays_distances) # get number of areas analyzed

# Calculate mean and std for each list
mean_heights_distances, std_heights_distances = calculate_mean_std_histograms(all_2nn_arrays_distances, bins, N)
mean_heights_distances_csr, std_heights_distances_csr = calculate_mean_std_histograms(all_2nn_arrays_distances_csr, bins, N)
...
# Save to CSV
df_data_stats.to_csv(os.path.join(main_dir, '2nn_data_histogram_stats.csv'), index=False)
df_csr_stats.to_csv(os.path.join(main_dir, '2nn_csr_histogram_stats.csv'), index=False)

# === Extension: Reload and replot mean ± std histogram stats for data and CSR ===

# Load stats from CSVs
df_data_stats = pd.read_csv(os.path.join(main_dir, '2nn_data_histogram_stats.csv'))
df_csr_stats = pd.read_csv(os.path.join(main_dir, '2nn_csr_histogram_stats.csv'))
