...
source_data_path = os.path.join(outputdir, 'Figure3h_SourceData.csv')
...
source_data = pd.DataFrame({
    ...
    'Condition': (['ManNAz'] * 4 +
                  ...
                  ['GalNAz_CSR'] * 4),
    'Mean': np.concatenate([
        all_counts_nsugars_manna[0][0][2:6],
        all_counts_nsugars_csr_manna[0][0][2:6],
        all_counts_nsugars_galna[0][0][2:6],
        all_counts_nsugars_csr_galna[0][0][2:6]
    ]),
    'SEM': np.concatenate([
        all_counts_nsugars_manna[0][1][2:6] / np.sqrt(K),
        all_counts_nsugars_csr_manna[0][1][2:6] / np.sqrt(K),
        all_counts_nsugars_galna[0][1][2:6] / np.sqrt(K),
        all_counts_nsugars_csr_galna[0][1][2:6] / np.sqrt(K)
    ])
})
source_data.to_csv(source_data_path, index=False)

# Replot from CSV (check)
df_check = pd.read_csv(source_data_path)

fig_check, ax_check = plt.subplots(figsize=(6, 5))
bar_width = 0.4
x = np.arange(4)
...
offsets = {
    'ManNAz': -bar_width / 2,
    'ManNAz_CSR': -bar_width / 2,
    'GalNAz': bar_width / 2,
    'GalNAz_CSR': bar_width / 2
}

for cond in df_check['Condition'].unique():
    subset = df_check[df_check['Condition'] == cond]
    ax_check.bar(
        x + offsets[cond],
        subset['Mean'],
        bar_width,
...
ax_check.set_xlim(-0.5, len(x) - 0.5)
ax_check.set_ylim(1e-4, 1)
ax_check.set_xlabel('N sugars per cluster')
ax_check.set_ylabel('Counts')
ax_check.set_xticks(x)
ax_check.set_xticklabels([2, 3, 4, 5], fontsize=12)
ax_check.tick_params(axis='both', which='both', direction='in')
ax_check.spines['bottom'].set_clip_on(True)
ax_check.spines['left'].set_clip_on(True)
ax_check.legend()
plt.tight_layout()
plt.show()
